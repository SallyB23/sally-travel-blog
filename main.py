...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(email=user_email).first()
        if user is None:
            error = 'This email address does not exist. Please try again.'
        else:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment(
                text=strip_invalid_html(form.body.data),
                author=current_user,
            ...
            error = 'You need to be logged in to leave a comment. Please log in.'
            return redirect(url_for('login', error=error))
